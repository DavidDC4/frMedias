...
public interface PartidoRepository extends CrudRepository<Partido, Long> {
	...
	@Query("SELECT p FROM Competicion c JOIN c.partidos p WHERE c.activo = ?1 AND p.fecha >= ?2 AND p.fecha <= ?3")
	List<Partido> buscarPartidos(boolean activo, Date fechaDesde, Date fechaHasta);

	@Query("SELECT p FROM Competicion c JOIN c.partidos p WHERE c = ?1")
